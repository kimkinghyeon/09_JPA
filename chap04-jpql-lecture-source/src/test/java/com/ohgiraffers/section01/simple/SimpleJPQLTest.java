...
public class SimpleJPQLTest {
    ...
    SQL 보다 간결하고, 툭종 DBMS 에 의존하지 않는다.
    방언을 통해 해당 DBMS 에 맞는 SQL 을 실행하게된다.
    JPQL 은 find() 메서드를 통한 조회와 다르게 항상 데이터베이스에서 SQL 을 실행해서 결과를 조회
    * */
    ...
    * */
    ...
    @DisplayName("TypeQuery 를 이용한 단일메뉴 조회 테스트")
    @Test
    public void test1() {
...
//then
    ...
    public void test3() {
        ...
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class); //반환타입
        Menu resultMenu = query.getSingleResult();//결과반환
        //then
    ...
    public void test4() {
        ...
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class); //반환타입
        List<Menu> menuList = query.getResultList();//결과반환
        //then
