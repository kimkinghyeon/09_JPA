...
public class ProjectionTests {
    ...
    엔티티 랑 거의 비슷하게 사용된다. (조회의 시작점이 될 수 없다.) -> from 에 사용불가
    3. 스칼라 타입 프로젝션
    숫자, 문자, 날짜 같은 기본 데이터 타입
    스칼라 타입은 영속성 컨텍스트에서 관리되지 않는다.
    4. new 명령어 활용한 프로젝션
    다양한 종류의 단순 값들을 DTO 로 조회하는 방식 (new 패키지명.DTO 명)
    -> NEW 를 사용해서 만들기 때문에 기본생성자가 필요하다.
    NEW 명령어를 사용한 클래스는 엔티티가 아니므로 영속성 컨텍스트에서 관리하지 않는다.
    * */

    @Test
    @DisplayName("단일 에티티 프로젝션 테스트")
    void test1() {
        //given
    ...
    public void test5() {
        ...
        categoryList.forEach(row -> {Arrays.stream(row).forEach(System.out::println);});
    }
