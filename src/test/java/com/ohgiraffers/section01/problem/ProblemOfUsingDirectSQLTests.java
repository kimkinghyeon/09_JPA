...
public class ProblemOfUsingDirectSQLTests {
    ...
    public ProblemOfUsingDirectSQLTests() throws SQLException {
    }
    ...
    /*
    JDBC API 를 이용해 직접 SQL 을 다룰 떄 발생 할 수 있는 문제점
    1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성
    2. SQL 에 의존하여 개발
    3. 패러다임 불일치(상속 , 연관 관계, 객체, 그래프 탐색)
    4. 동일성 보장문제
    * */
    ...
    void testAssociatedObject() throws SQLException {
        ...
        List<MenuCategory> menuAndCategories = new ArrayList<>();
        while(rset.next()) {
            MenuCategory menuAndCategory = new MenuCategory();
