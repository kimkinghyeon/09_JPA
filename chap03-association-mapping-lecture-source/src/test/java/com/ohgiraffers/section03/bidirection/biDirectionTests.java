...
public class biDirectionTests {
    ...
    양방향 연관관계는 항상 설정하지 않고, 반대 방향으로도 접근하여 객체 그래프 탐색을
    할일이 많은 경우에만 사용한다.
    양방향 매핑에서 연관관계의 주인은 외래키를 가지고있는 엔티티이다.
    * */
    ...
    public void test1() {
        ...
        // oneToMany 연관관계는 필요시 연관관계 엔티티를 조회하는 쿼리를 다시실행한다.
        Category foundCategory = entityManager.find(Category.class,categoryCode);

        //then
        // toString() 오버라이딩 또는 lombok 을 사용하면 양방향 연관관계에서는
        // 재귀호출이 일어나기 때문에 stackOverFlow 발생!
        // menuList <= toString 무한 호출
