...
public class EmbeddedKeyTests {
    ...
    복합키가 존재하는 테이블의 매핑
    1. Embedded
    @Embeddable 클래스에 복합키를 정의하고 언티티에 @EmbeddedId 를 이용하여 매핑
    복합키의 일부 필드만을 매핑할 수 있다. 따라서 필드 수가 많은 경우 유연하게 매핑이 가능하다.
    2. IdClass
    복합키를 필드로 정의한 클래스를 이용해서 클래스에 @IdClass 를 매핑
    복합키를 구성하는 모든 필드를 한번에 매핑이 가능하다.
    코드가 간결하다.

    @EmbeddedId 가 조금 더 객체지향적인 방식이고, IdClass 가 관계형 데이터 베이스가 가까운 방식이다.

    두방식 모두 복합키 클래스 영속성 클래스가 관리하지 않는다는 특징을 가진다.
    ...
    2.@GeneratedValue 어노테이션을 사용해서 복합키를 자동으로 생성하는것은 불가능
            */
    @DisplayName("임베이디드 아이디 사용한 복합키 테이블 매핑 테스트")
    @Test
